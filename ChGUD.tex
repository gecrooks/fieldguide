...
%\SSec{Special cases: Extended Pearson}
\SSec{Special cases}
...
\hline
\eqref{ExtPearson} & Ext. Pearson  &.&.&.&.&.&.&.&.&$1$\\
\eqref{Pearson}	 & Pearson  &.&.&0&.&.&.&.&.&$1$\\
\eqref{GenBeta} & gen. beta &.&.&.&.&0&0&1&-1&.\\
\eqref{GenBeta} & gen. beta prime &.&.&.&.&0&0&1&1&.\\
\eqref{InvGaussian}	 & inv. Gaussian  &0&1&.&.&.&0&.&0&1 \\	
\eqref{InvGaussian}	 & inv. Gaussian &.&.&.&-$\half$&.&0&1&0&1 ??\\	
\eqref{Halphen} & Halphen &.&.&$\kappa$&.&-$\kappa$&0&1&0&1 \\
\eqref{Hyperbola} & Hyperbola &.&.&$\kappa$&-1&-$\kappa$&0&1&0&1 \\

\eqref{Sichel} & Sichel &.&.&.&.&.&0&1&0&1 \\
\end{tabular} 
}
\end{center}
\end{table*}
...
\begin{align}
\label{InvGaussian}
\opr{InvGaussian}(x\given \mu,\lambda) &= \sqrt{\frac{\lambda}{2\pi x^3}} \exp\left(\frac{-\lambda (x-\mu)^2}{2\mu^2 x}\right)
\\ \notag
& = \opr{ExtPearson}(x\given 0,1\ ; \ \lambda\mu^2, -3 \mu^2, -\lambda \ ; \ 0, 2 \mu^2,0 ) 
\\ \notag
& = \opr{GUD}(x\given 0,1\ ; \ \lambda\mu^2, -3 \mu^2, -\lambda \ ; \ 0, 2 \mu^2,0 \ ; \ 1) 
\end{align}
...
\[
\lim_{\mu\rightarrow\infty} \opr{InvGaussian}(x\given \mu, \lambda) = \Levy(x\given 0,\lambda)
\notag
\]
...
\\
& \sim \opr{InvGaussian}\Bigl(x\given \mu' \sum_i w_i, \lambda' \bigl(\sum_i w_i\bigr)^2 \Bigr)
\end{align*}

Scaling an inverse Gaussian scales both $\mu$ and $\lambda$. 
\[
c\ \opr{InvGaussian} (\mu, \lambda) \sim \opr{InvGaussian} (c \mu, c \lambda)
\notag
\]

It follows from the previous two relations the sample mean of an inverse Gaussian is inverse Gaussian.
\[
\frac{1}{N} \sum_{i=1}^N \opr{InvGaussian}_i (\mu, \lambda) \sim \opr{InvGaussian} (\mu, N \lambda)
\notag 
\]
...
\notag
\\
 &=  \op{GUD}(x\given a,s \sep \kappa, \alpha-1 , -\kappa\sep 0,1,0 \sep 1)
\notag
\checked
...
\notag
\\
& = \op{GUD}(x\given a,s\sep \alpha-1, \kappa,  -2\sep 1,0,0\sep 1)
\notag
\\& \qquad  0 \leq \tfrac{x-a}{s}
\notag
\end{align}
...
\notag
\\
& = \op{GUD}(x; a,s; 2, -\kappa , -\alpha+1; 0,0,1;1)
\notag
\\
& = \op{GUD}(x ; a,s; \alpha-1, \kappa,  -2; 0,0,1;-1)
\notag
\\& \qquad  0 \leq \tfrac{x-a}{s}
\notag
\end{align}
...
\checked
\\
& = \op{GUD}(x \given a,s \sep \lambda, \alpha-1 , -\kappa \sep 0,1,0 \sep 1)
\checked
\notag
\\& \qquad  0 \leq \tfrac{x-a}{s} \notag
\end{align}
Special cases include Halphen \eqref{Halphen} $\lambda= \kappa$, and inverse Gaussian \eqref{InvGaussian} $\alpha = -\half$. 
...
\notag
\\ \notag
& = \op{GUD}(x|a,s; 1-\alpha, \alpha + c + c\gamma-3, 2- 2c; 
...
&\opr{GaussHypergeometric}(x\given a,s,u, \alpha, \gamma, \rho) 
\\ \notag
& =   \frac{1}{|s| \mathcal{N}} 
...
\\ \notag &
=\op{GUD}(x\given a, s \sep \alpha-1,2 - \alpha -\gamma + (1-u)(1+ \rho+\alpha),\\
& \qquad\qquad \qquad\notag
u(\alpha+\gamma-\rho-2) \sep 1, -1-c, -u \sep 1) %CHECKME
\\ \notag
& \qquad 0\leq \tfrac{x-a}{s} \leq1
\end{align}
...
\notag
\\
& = GUD(a,s; \kappa, \alpha-1 , -\kappa; 0,1,0;\beta) \checked
\notag
\\
& \qquad0 \leq \tfrac{x-a}{s} \leq 1 \notag
\end{align}
...
\label{Laha}
	...
	\\ & = \opr{GUD}^{(4)}(x \given  a,s\ ;\ 0,0,0,0,4\ ;\ 1,0,0,0,1\ ;\ 1)  \checked \notag
\end{align}
...
% an extended Pearson \eqref{ExtPearson} distribution, 
