...
\dist{Generalized Sichel} (generalized generalized inverse Gaussian) distribution~\cite{Shakil2010}:
\begin{align}
\label{GenSichel}
...
\begin{table*}[tp]
\begin{center}
\caption[Pearson-exponential Distribution -- Special cases]{Special cases of the Pearson Exponential Family}
\label{PearsonExp}
\begin{tabular}{llcccccccccc}
...
% \eqref{Logistic} & Logistic    &.&.&-1&0&0&1&0&0 \\
% \eqref{HyperbolicSecant} & Hyperbolic secant    &.&.&-1&0&0&1&0&0 \\
\end{tabular} 
\end{center}
\end{table*}
...
If we take the limit of $\beta$ to infinity (See \secref{???}), then we get the family of Pearson exponential distributions.

\dist{Pearson-exponential} distribution~\cite{\self}:
\begin{align}
\opr{PearsonExp}&(x\given  \pLoc, \pScale; a_0, a_1,a_2;  b_0,b_1,b_2)
 \label{PearsonExp}
...
\notag
\end{align}
See table~\ref{PearsonExp} and Fig.~\ref{PearsonExpHierarchy}.
